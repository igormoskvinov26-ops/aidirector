...
async def main() -> None:
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=30.0) as client:
        ...
        тело = ответ.json()
        филиалы = тело.get("data") or []

    if not филиалы:
        print("Ключи приняты, но ни одного филиала не вернулось.\n")
        # Печатаем ответ целиком: гадать о причине по пустому списку нечем,
        # а в теле обычно лежит и признак успеха, и текст ошибки.
        print("Что ответил YCLIENTS:")
        print(json.dumps(тело, ensure_ascii=False, indent=2)[:1200])
        print(
            "\nЧаще всего дело в пользовательском токене. Партнёрский берётся\n"
            "из настроек YCLIENTS, а пользовательский там не показывают — его\n"
            "получают отдельным запросом. Сделать это можно так:\n"
            '    uv run python "../scripts/получить-токен.py"'
        )
        sys.exit(1)
