...
const box = 'rounded-2xl border border-milk-line dark:border-line bg-milk-card dark:bg-panel p-5';
export default function BarberMonthPage({payroll = false}: {payroll?: boolean}) {
  ...
  return <section className="space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-4"><div>
      ...
      <p className="mt-2 text-sm text-muted-light dark:text-muted">{data ? `Текущий месяц: ${date(data.month_start)} — ${date(data.month_end)}` : 'Текущий месяц · Москва'}</p>
    </div><button disabled={busy} onClick={() => setRevision(v => v + 1)} className="rounded-xl border border-milk-line dark:border-line px-4 py-2 disabled:opacity-50">{busy ? 'Обновление…' : 'Обновить'}</button></header>
    {error && <p role="alert" className="rounded-xl bg-loss/10 p-4 text-loss">{error} {data && 'Ниже — данные последнего успешного обновления.'}</p>}
    {data?.warnings.map(w => <p key={w} role="status" className="rounded-xl bg-caution/10 p-4 text-caution">{w}</p>)}
    <p className="text-sm text-muted-light dark:text-muted">{payroll ? 'За день: большее из двух — 40% услуг + 10% косметики или гарант за рабочую смену. Сегодняшняя сумма предварительная. Прогноз учитывает текущие записи и опубликованные смены, но не новые записи и будущие продажи.' : 'Выполненные визиты и косметика — с начала месяца. Будущие записи — с текущего момента до конца месяца. Отмены и неявки исключены.'}</p>
    ...
    {payroll && data?.totals && data.scope === 'all' && <div className={box}>
      <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-muted-light dark:text-muted">Итого по всем мастерам</h2>
      <dl className="grid grid-cols-2 gap-5 lg:grid-cols-4">
        <div><dt className="text-sm text-muted-light dark:text-muted">Начислено</dt><dd className="mt-1 text-2xl font-semibold text-bronze dark:text-gold">{rub(data.totals.earned)}</dd></div>
        <div><dt className="text-sm text-muted-light dark:text-muted">Прогноз за месяц</dt><dd className="mt-1 text-xl">{rub(data.totals.forecast)}</dd></div>
        <div><dt className="text-sm text-muted-light dark:text-muted">Выполнено записей</dt><dd className="mt-1 text-xl">{data.totals.completed_count} на {rub(data.totals.completed_revenue)}</dd></div>
        <div><dt className="text-sm text-muted-light dark:text-muted">Будущих записей</dt><dd className="mt-1 text-xl">{data.totals.future_count} на {rub(data.totals.future_revenue)}</dd></div>
      </dl>
    </div>}
    ...
    <div className="grid gap-4 xl:grid-cols-3">{data?.masters.map(m => <article key={m.staff_id} className={box}>
      ...
      {payroll ? <dl className="space-y-5">
        ...
        <div><dt className="text-sm text-muted-light dark:text-muted">Гарант за смену</dt><dd>{rub(m.rule?.guarantee)}</dd></div>
    ...
    {payroll && <p className="text-xs text-muted-light dark:text-muted">* Сегодняшний гарант рассчитан при условии полной смены. Суммы до удержаний и выплат. Прогноз не гарантирует получение выручки; смены, ещё не внесённые в YCLIENTS, не учтены.</p>}
    {data && <p className="text-xs text-muted-light dark:text-muted">Данные получены {new Date(data.updated_at).toLocaleString('ru-RU', {timeZone:'Europe/Moscow'})} МСК · автоматическое обновление раз в минуту</p>}
  </section>;
}
function Metric({label, value}: {label: string; value: string}) {
  return <div className="flex items-baseline justify-between gap-4"><dt className="text-sm text-muted-light dark:text-muted">{label}</dt><dd className="text-lg font-semibold whitespace-nowrap">{value}</dd></div>;
}
