...
Решение владельца 23.09.2026: лояльный — 2–3 визита к одному мастеру,
постоянный — 4 и больше, сегменты непересекающиеся. Клиент приписан к
...
def test_четыре_визита_это_постоянный():
...
def test_постоянный_не_считается_заодно_лояльным():
    ...
    сегмент, _ = classify_client({КСЕНИЯ: _дни(50, 40, 30, 10)}, БАРБЕРЫ, False, moscow_today())
...
def test_потеряли_постоянного_столбец_остаётся_за_мастером():
    """Ксения потеряла клиента с четырьмя визитами — это её столбец,
    а не «ничей»: вопрос к тому, кто его вёл."""
    итог = classify_client({КСЕНИЯ: _дни(200, 190, 180, 170)}, БАРБЕРЫ, False, moscow_today())
    assert итог == ("lost", КСЕНИЯ)
...
def test_много_визитов_но_все_к_разным_мастерам_это_постоянный_без_мастера():
    """Четыре визита в салон, ни к кому двух — постоянный клиент салона,
    но ничей. Ровно тот случай, ради которого заведён четвёртый столбец."""
    визиты = {КСЕНИЯ: _дни(50), АРТАШ: _дни(40), ДМИТРИЙ: _дни(30), АДМИНИСТРАТОР: _дни(10)}
    итог = classify_client(визиты, БАРБЕРЫ, False, moscow_today())
    assert итог == ("regular", NO_MASTER)


def test_свой_мастер_это_тот_у_кого_чаще():
    визиты = {КСЕНИЯ: _дни(60, 50, 40, 30), АРТАШ: _дни(10)}
    итог = classify_client(визиты, БАРБЕРЫ, False, moscow_today())
    assert итог == ("regular", КСЕНИЯ)
